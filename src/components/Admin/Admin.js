...
const Admin = () => {
  ...
  const deleteBook=id=>{
    fetch(`https://stark-journey-80362.herokuapp.com/delete/${id}`,{
      method:'DELETE'
    })
    .then(res => res.json())
    .then(result => {
            ...
            <div className="w-25 border border-primary mr-5 h-100">
                <Navbar>
                    <Nav className="d-block">
                        ...
                        <Nav.Link onClick={() => setManage(!manage)}><MdBorderAll/> Manage Admin</Nav.Link>
                        <Nav.Link onClick={() => setManage(!manage)}><FaPlus/> Add Book</Nav.Link>
